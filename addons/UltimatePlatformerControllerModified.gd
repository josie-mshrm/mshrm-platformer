...
class_name PlatformerController2DCustom
...
func _updateData():
	...
	dashMagnitude = maxSpeed * dashLength # the magnitude of the dash vector, maximum speed times the length of the dash in time
	dashCount = dashes
...
func dash_eightway(): # this entire function is running in the physics process
	...
	if eightWayDash and dashTap and dashCount > 0 and !rolling:
		...
		#print(lastdash_direction)
		
	if (lastdash_direction.y >= 0.7 or lastdash_direction.y <= -0.7) and (lastdash_direction.x >= 0.7 or lastdash_direction.x <= -0.7):
		diagonaldash = true
	else:
		diagonaldash = false
		# x < -0.6 is left, x > 0.6 is right
		# y > 0.6 is down, y < -0.6 is up 
	
	if dashing:
		if is_on_floor() and diagonaldash and !wavedash: # if the player is on the floor, the last dash was diagonal, and wavedash state isn't already active
			_wavedashTime(jumpBuffering/2) # time alloted to the wavedash is linked to the jump buffer time
	
	# if the player dashes down diagonally and hits the ground while in the dash state
	# then if the player hits jump within (time) while this "super" state is true
	# apply the crazy speed boost

func dashcheck_diagonal() -> bool:
	if lastdash_direction.y > 0.7 or lastdash_direction.y < -0.7:
		return true
	return false

func _wavedashTime(time):
	wavedash = true
...
func _set_jumpMagnitude() -> void:
	...
	if !wavedash:
		...
		wavedashMagnitude = velocity.x
	elif wavedash:
		_pauseGravity(jump_fall_time/4)
		jumpMagnitude = -jump_velocity * gravityScale * 0.4
		wavedashMagnitude += -jump_velocity * gravityScale * (lastdash_direction.x * 10)
